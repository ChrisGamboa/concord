...
/**
 * Detects an active @query in the input and shows a filtered member dropdown.
 * Returns null if no @ trigger is active.
 */
export function MentionAutocomplete({ members, input, cursorPos, onSelect, onClose }: Props) {
  const [activeIndex, setActiveIndex] = useState(0);
  const listRef = useRef<HTMLDivElement>(null);

  // Find the @ trigger: look backwards from cursor for an unescaped @
  ...
  const handleSelect = (member: MentionMember) => {
    onSelect(member.username, trigger.startIdx, trigger.endIdx);
  };
  ...
  return { filtered, activeIndex, handleSelect, handleKeyDown, listRef };
}
